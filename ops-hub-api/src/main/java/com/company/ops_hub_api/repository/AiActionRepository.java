...
public interface AiActionRepository extends JpaRepository<AiAction, Long> {
    ...
    @Query("SELECT a FROM AiAction a WHERE a.conversation.id = :conversationId AND a.actionType = :actionType ORDER BY a.createdAt DESC")
    List<AiAction> findRecentByConversationAndActionType(@Param("conversationId") Long conversationId,
